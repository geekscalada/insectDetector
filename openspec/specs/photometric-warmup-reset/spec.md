...
Cuando el filtro fotométrico detecta un cambio global (`mean_diff > photometric.max_mean_diff`), el módulo `detector` SHALL, además de devolver `[]` y aplicar MOG2 con `learningRate=0`, resetear `_frame_count = 0` para forzar re-warmup completo y actualizar `_prev_gray` al frame actual. SHALL establecer el flag interno `_photometric_event = True` para que el `orchestrator` pueda consultarlo mediante la función pública `last_photometric_event() -> bool`. El flag SHALL resetearse a `False` al inicio de cada llamada a `detect()`.
...
#### Scenario: Inestabilidad fotométrica actualiza prev_gray para estabilizarse
- **WHEN** `detect(frame)` detecta un evento fotométrico
- **THEN** `_prev_gray` se actualiza al frame actual para que frames consecutivos al mismo nivel de exposición dejen de disparar el filtro
...
- **THEN** `debug.py` imprime `[!] CAMBIO FOTOMÉTRICO — reiniciando warmup (diff: X.X)` y guarda jpg con sufijo `fondo_cambiado`
